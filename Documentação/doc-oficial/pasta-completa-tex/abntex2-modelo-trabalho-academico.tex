...
\begin{itemize}
	...
	\item Projetar a arquitetura do sistema conforme o padrão \textit{Model-Template-View} (MTV) do framework Django;
	...
	\item Aplicar recursos de segurança e privacidade de dados, tais como autenticação de usuários, criptografia de comunicações via protocolo HTTPS;
...
\begin{quadro}[H]
	...
	\begin{tabular}{|c|p{6.5cm}|p{3.8cm}|c|}
		...
		\textit {Check-in} e \textit {Check-out} &	ALTA
		 \\ \hline
		12 & Integrar ambientes \textit{backend} e \textit {frontend} &	Requisito técnico & ALTA 
		\\ \hline
...
Neste contexto, para auxiliar a equipe na modelagem e desenvolvimento aplicação, foi realizado o levantamento das regras de negócio do sistema web de gestão da pousada Chalés Água de Coco, disponíveis no \autoref{quadro_rn_1}. 
  \\
%
\begin{quadro}[H]
...
Essa infraestrutura garante redundância, políticas de backup e suporte a conexões seguras via HTTPS, fatores indispensáveis para um sistema que manipula dados pessoais e financeiros.

\subsection{Boas Práticas de Desenvolvimento}

Além da escolha tecnológica, adotaram-se boas práticas de engenharia de software, como versionamento de código com Git, integração e entrega contínuas (CI/CD), aplicação de testes automatizados e atualizações periódicas de dependências. Tais práticas favorecem a manutenibilidade, reduzem riscos de falhas em produção e facilitam a evolução da aplicação, assegurando qualidade e estabilidade ao longo de seu ciclo de vida.
...
O código demonstra boa legibilidade, utilizando identação consistente, nomes descritivos e separação lógica de blocos. Comentários são empregados para explicar trechos mais complexos, tanto no backend, quanto no frontend.
...
Em resumo, o projeto apresenta uma codificação consistente, modular e alinhada às boas práticas da comunidade Django e Python. A separação em apps, a organização interna dos módulos e o uso de recursos nativos do framework favorecem a manutenibilidade e a evolução do sistema.
...
O código-fonte do sistema foi estruturado em aplicações modulares, arquivos de configuração geral do ambiente \textit{Django} e arquivos auxiliares, seguindo o padrão recomendado pelo \textit{framework}. Essa organização, que visa facilitar o desenvolvimento, a manutenção e a segurança do sistema, está estruturada como  ilustrado na \autoref{fig:estrutura_rep}
%
\begin{comment}		
\begin{enumerate}
...
\begin{figure}[H]
	...
	\includegraphics[height=0.9\textheight]{estrutura_pastas.png}
	\fonte{Elaboração própria.}
\end{figure}
%
Essa configuração segue as boas práticas do \textit{framework}, promovendo baixo acoplamento e alta coesão. Na camada de lógica, essa coesão foi aplicada especificamente no \textit{app} "core", que organiza os arquivos models e views das entidades quarto, hóspede e reserva, as quais formam o domínio central do negócio: a gestão de reservas. Os demais \textit{apps} representam preocupações separadas: usuários (autenticação), financeiro (contabilidade) e relatórios (camada de leitura e agregação). E, por isso, seguiram uma lógica de baixo acoplamento.
 
No entanto, para a camada de apresentação (templates), optou-se por centralizar todos os arquivos em um único diretório (code/setup/templates/). Essa escolha, apesar de reduzir a portabilidade individual de cada app, aumenta a coesão do projeto e facilita a manutenção e a organização das interfaces, simplificando o fluxo de trabalho. 
...
Neste tópico é detalhado o planejamento e execução dos testes do sistema web de gestão da pousada. 
...
Para a realização da análise estática do código foi  utilizada a ferramenta SonarQube. O objetivo foi  identificar e corrigir \textit{code smells}, \textit{bugs} em potencial e vulnerabilidades de segurança antes do deploy.
...
\begin{quadro}[H]
	\adjustbox{max width=\textwidth}{
		...
		\label{quadro_tf4}
		\begin{tabular}{|c|p{3cm}|c|p{3cm}|p{4.5cm}|p{4.5cm}|}
			\hline
			\textbf{ID} & \textbf{Descrição} & \textbf{RF} & \textbf{Pré-condição} & \textbf{Ação} & \textbf{Resultado Esperado} \\ \hline
...
\begin{figure}[H]
	\centering
	\caption{QR Code Documento de Testes Funcionais}
...
Os testes unitários foram a primeira linha de defesa da qualidade do código e auxiliaram na validação da lógica interna da aplicação. Para o desenvolvimento, utilizou-se o \textit{framework} Pytest. Seguindo a convenção padrão do Django, os testes foram organizados em pastas \textit{tests/} localizadas dentro de cada \textit{app} (módulo) da aplicação. Esta estrutura permitiu a criação de testes limpos, modulares e eficientes.

Como estratégia, o foco não foi  desenvolver testes unitários capazes de atingir 100\% de cobertura de linhas , mas sim em garantir a robustez das áreas de maior complexidade e risco de negócio. O esforço foi concentrado em:
\begin{itemize}
	\item \textbf{Lógica de Cálculo:} Funções puras responsáveis pela geração de relatórios e KPIs (ex: balanço financeiro).
	\item \textbf{Funções de Validação:} A lógica complexa de validação nos \textit{forms.py} e \textit{models.py} (ex: regras de conflito de reserva).
	\item \textbf{Funções Utilitárias:} Componentes críticos de \textit{utils.py} (ex: exportação de arquivos).
\end{itemize}

O código padrão gerado pelo Django (como o Admin, migrações, ou funções de \textit{views.py} que apenas renderizam \textit{templates}) foi intencionalmente considerado de baixo risco e não foi o foco principal desta camada de testes, sendo validado pelos Testes Funcionais.
...
\begin{table}[H]
	...
	\adjustbox{max width=\textwidth}{
		\begin{tabular}{|>{\RaggedRight\arraybackslash}p{5.5cm}|c|c|c|c|}
			...
			\hline
			Engenheiro de Dados (DBA)        & 1 & 3 & 22 & 66 \\
			\hline
			Analista de Documentação         & 1 & 3 & 22 & 66 \\
			\hline
			Gerente de Projeto (PM)          & 1 & 3 & 22 & 66 \\
			\hline
			Desenvolvedor \textit{Front-End}          & 1 & 3 & 22 & 66 \\
			\hline
			Desenvolvedor \textit{Back-End}           & 1 & 3 & 22 & 66 \\
			\hline
		\end{tabular}
	}
	\fonte{Elaborado pelos autores.}
...
\begin{table}[H]
	...
	\adjustbox{max width=\textwidth}{
		\begin{tabular}{|l|r|r|r|}
			...
			\hline
			Estrutura& 940,50 & 3.762,80  & 8.464,50 \\
			\hline
			\textbf{Total} & 7.740,48 & 30.962,72 &  69.664,32\\
			\hline
		\end{tabular}
	}
	\fonte{Elaborado pelos autores.}
...
Para além disso, o desenvolvimento de um modelo SaaS permite custos previsíveis e escalabilidade, dado que é um modelo de disponibilização de software em nuvem, o que permite o acesso dos usuários à aplicação via internet a partir de um pagamento periódico (Correia, 2024). Além de ser um modelo que independe de estruturas de operação e manutenção robustas para o usuário final, que consegue ter acesso a infraestrutura de software a um baixo custo, o que facilita a adesão de ferramentas tecnológicas para empresas de diferentes portes \cite{Correia2024SaaS}. Sendo assim, conclui-se que este é um sistema financeiramente sustentável tanto para os desenvolvedores e investidores quanto para os futuros clientes.
...
25° SEMANA
Período: 12/11/2025 a 22/11/2025.
...
%
\begin{quadro}[H]
	\caption{Questionário Aplicado à Proprietária 2 – Parte 1}
	\label{quadro:questionario2_1}
	\vspace{0.3cm}
	{\bfseries\large Questionário Aplicado à Proprietária 2 – Parte 1} \\
	\vspace{0.3cm}
	\begin{tabular}{|>{\centering\arraybackslash}p{1cm}|p{6.5cm}|p{6.5cm}|}
		\hline
		\textbf{Item} & \textbf{Pergunta} & \textbf{Resposta} \\ \hline
...
\begin{quadro}[H]
	...
	\begin{tabular}{|>{\centering\arraybackslash}m{1.4cm}|>{\raggedright\arraybackslash}m{6.5cm}|>{\centering\arraybackslash}m{3.8cm}|>{\centering\arraybackslash}m{2.5cm}|}
		\hline
		\textbf{Código} & \textbf{Item} & \textbf{Categoria} & \textbf{Prioridade} \\ \hline
...
\begin{quadro}[H]
	\caption{Sprints Backlog - Parte 1} 
	\label{sprints_backlog_1} 
	\begin{tabular}{|>{\centering\arraybackslash}m{1.2cm}|>{\centering\arraybackslash}m{2.5cm}|>{\centering\arraybackslash}m{3cm}|>{\raggedright\arraybackslash}m{7.5cm}|}
		\hline
		\textbf{Sprint} & \textbf{Período} & \textbf{Objetivo} & \textbf{Atividades} \\
		\hline 
...
%
\begin{quadro}[H]
	\caption{Sprints Backlog - Parte 5} 
	\label{sprints_backlog_5} 
	\begin{tabular}{|>{\centering\arraybackslash}m{1.2cm}|>{\centering\arraybackslash}m{3cm}|>{\centering\arraybackslash}m{3cm}|>{\raggedright\arraybackslash}m{7cm}|}
		\hline
		\textbf{Sprint} & \textbf{Período} & \textbf{Objetivo} & \textbf{Atividades} \\
		\hline 
...
%
\begin{quadro}[H]
	\caption{Sprints Backlog - Parte 6} 
	\label{sprints_backlog_6} 
	\begin{tabular}{|>{\centering\arraybackslash}m{1.2cm}|>{\centering\arraybackslash}m{3cm}|>{\centering\arraybackslash}m{3cm}|>{\raggedright\arraybackslash}m{7cm}|}
		\hline
		\textbf{Sprint} & \textbf{Período} & \textbf{Objetivo} & \textbf{Atividades} \\
		\hline 
...
\end{quadro}
\begin{quadro}[H]
	\caption{Sprints Backlog - Parte 7} 
	\label{sprints_backlog_7} 
	\begin{tabular}{|>{\centering\arraybackslash}m{1.2cm}|>{\centering\arraybackslash}m{3cm}|>{\centering\arraybackslash}m{3cm}|>{\raggedright\arraybackslash}m{7cm}|}
		\hline
		\textbf{Sprint} & \textbf{Período} & \textbf{Objetivo} & \textbf{Atividades} \\
		\hline 
...
%
\begin{quadro}[H]
	\caption{Sprints Backlog - Parte 8} 
	\label{sprints_backlog_8} 
	\begin{tabular}{|>{\centering\arraybackslash}m{1.2cm}|>{\centering\arraybackslash}m{3cm}|>{\centering\arraybackslash}m{3cm}|>{\raggedright\arraybackslash}m{7cm}|}
		\hline
		\textbf{Sprint} & \textbf{Período} & \textbf{Objetivo} & \textbf{Atividades} \\
		\hline  
...
\textbf{Data da Execução:} 29 de outubro de 2025 \\
\textbf{Testador:} Guilherme Bittencourt Schmidt \\
