...
%
\begin{quadro}[H]
	\caption{\label{quadro_rn_1}Regras de Negócio- Parte 1}
	\begin{tabular}{|>{\centering\arraybackslash}p{3cm}|p{11cm}|}
		...
		RN08 & Os horários padrão de \textit{check-in} e \textit{check-out} são: \textit{check-in} das 16h às 22h; \textit{check-out} das 8h às 14h. Em períodos de temporada os horários mudam para: \textit{check-in} das 15h às 22h; \textit{check-out} das 8h às 11h.\\ \hline
...
\begin{quadro}[H]
	...
	\begin{tabular}{|>{\centering\arraybackslash}p{3cm}|p{11cm}|}
		...
		RN17 & São permitidos \textit{early check-in} e \textit{late check-out} com cobrança de tarifas adicionais.   \\ \hline
...
\begin{quadro}[H]
	...
	\begin{tabular}{|>{\centering\arraybackslash}p{1.5cm}|p{6.7cm}|>{\centering\arraybackslash}p{2cm}|>{\centering\arraybackslash}p{4cm}|}
		...
		RF04 & O sistema deve permitir o cadastro de novas reservas, associando um quarto a um período (data de \textit{check-in} e \textit{check-out}) & Alta & RN04; RN11 \\ \hline
...
\begin{quadro}[H]
	...
	\begin{tabular}{|>{\centering\arraybackslash}p{1.5cm}|p{6.7cm}|>{\centering\arraybackslash}p{2cm}|>{\centering\arraybackslash}p{4cm}|}
		\hline
		\textbf{Código} & \textbf{Descrição} & \textbf{Prioridade} & \textbf{Regra de Negócio Relacionada} \\ \hline
		RF20 & A proprietária deve conseguir editar as informações dos quartos já cadastrados & Alta & RN07; RN10\\ \hline
...
\begin{quadro}[H]
	...
	\begin{tabular}{|p{1cm}|p{6.5cm}|p{6.5cm}|}
		...
		1 & Há um prazo mínimo ou máximo para fazer uma reserva? & Preferencialmente, antecipadamente. Nas plataformas coloco 2 dias de antecedência. \\ \hline
		2 & A reserva é confirmada apenas com pagamento ou pode ser feita sem pagamento antecipado? & Confirmada pelo pagamento de pelo menos metade do valor da reserva. \\ \hline
		3 & É permitido cancelar uma reserva? Até quantas horas antes do check-in? Há cobrança de taxa? & Sim! Temos políticas de cancelamentos e remarcações. \\ \hline
		4 & Um hóspede pode fazer mais de uma reserva ativa ao mesmo tempo? & Sim. \\ \hline
		5 & Quais dados do hóspede são obrigatórios para fazer uma reserva? & Nome completo, endereço completo, CPF, telefone, e-mail. \\ \hline
		6 & É comum haver reservas feitas por um responsável em nome de outros hóspedes? & Sim. \\ \hline
		7 & Há um limite de pessoas por quarto? Como isso é controlado? & No check-in. \\ \hline
		8 & Qual é o horário padrão de check-in e check-out? Há tolerância? & Check-in a partir das 16h até às 22h e check-out das 8h até às 14h. Depende de se há entrada de outro hóspede em seguida. \\ \hline
		9 & O check-in pode ser feito antes do horário? E o check-out após o horário? & Depende, se houver saída de hóspede anterior ou entrada em seguida. \\ \hline
		10 & Quem realiza o check-in e check-out? Você ou os funcionários? & Eu ou sozinhos, com orientações minhas. \\ \hline
		11 & Há necessidade de gerar comprovante ou recibo após check-in ou check-out? & Não. \\ \hline
		12 & A pousada possui quantos quartos? Como eles são classificados? & 16. Quartos simples para casal (com e sem ar), chalés com cozinha para até 4 pessoas, com e sem ar, e flats, com e sem ar. \\ \hline
	\end{tabular}
	\fonte{Elaborado pelos autores.}
\end{quadro}
\begin{quadro}[H]
	...
	\begin{tabular}{|p{1cm}|p{6.5cm}|p{6.5cm}|}
		\hline
		\textbf{Item} & \textbf{Pergunta} & \textbf{Resposta} \\ \hline
		13 & Há períodos em que quartos são bloqueados para manutenção? & Sim. \\ \hline
		14 & Um mesmo quarto pode ser reservado para diferentes hóspedes em dias seguidos? & Sim. \\ \hline
		15 & A pousada oferece serviços extras? & Não. \\ \hline
		16 & Quais formas de pagamento são aceitas (Pix, cartão, dinheiro)? & As três formas, porém no cartão há taxa da operadora. \\ \hline
	\end{tabular}
	\fonte{Elaborado pelos autores.}
\end{quadro}
% ----------------------------------------------------------
